...
var _default = _table["default"].create('glyf', [], {
  read: function read(reader, ttf) {
    ...
    var glyphs = [];
    reader.seek(startOffset); // subset
    ...
    if (subset && subset.length > 0) {
      ...
      subsetMap[0] = true; // subset map

      var cmap = ttf.cmap; // unicode to index
      ...
      var parsedGlyfMap = {}; // 循环解析subset相关的glyf，包括复合字形相关的字形

      var travelsParse = function travels(subsetMap) {
        var newSubsetMap = {};
        Object.keys(subsetMap).forEach(function (i) {
          var index = +i;
          parsedGlyfMap[index] = true; // 当前的和下一个一样，或者最后一个无轮廓
